...
TEST_CASE("no items", "[maximumValue][3993a824-c20e-493d-b3c9-ee8a7753ee59]") {
    int                               max_weight = 100;
    std::vector<knapsack::Item> const items{};
...
TEST_CASE(
    ...
    int                               max_weight = 10;
    std::vector<knapsack::Item> const items{{100, 1}};
    REQUIRE(knapsack::maximum_value(max_weight, items) == 0);
}
...
TEST_CASE(
    ...
    int max_weight = 10;
    std::vector<knapsack::Item> const
        items{{2, 5}, {2, 5}, {2, 5}, {2, 5}, {10, 21}};
...
TEST_CASE(
    ...
    int max_weight = 10;
    std::vector<knapsack::Item> const
        items{{2, 20}, {2, 20}, {2, 20}, {2, 20}, {10, 50}};
    REQUIRE(knapsack::maximum_value(max_weight, items) == 80);
}

TEST_CASE(
    "example knapsack", "[maximumValue][81d8e679-442b-4f7a-8a59-7278083916c9]"
) {
    int                               max_weight = 10;
    std::vector<knapsack::Item> const items{{5, 10}, {4, 40}, {6, 30}, {4, 50}};
...
TEST_CASE("Edge case", "[maximumValue]") {
    ...
    REQUIRE(knapsack::maximum_value(max_weight, items) == 6);
}
