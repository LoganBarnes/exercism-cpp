...
TEST_CASE(
    "encode single characters only are encoded without count", "[encode]"
) {
...
TEST_CASE(
    "encode single characters mixed with repeated characters", "[encode]"
) {
    REQUIRE(
...
TEST_CASE("encode and then decode 2", "[consistency]") {
    constexpr auto const* original = "zzz ZZ  zZ";

    auto const encoded = run_length_encoding::encode(original);
    REQUIRE(encoded == "3z 2Z2 zZ");
    REQUIRE(run_length_encoding::decode(encoded) == original);
}
#endif
