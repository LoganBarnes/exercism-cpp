...
inline auto uv_light_heuristic(std::vector<int> const* const data) {
    ...
    return static_cast<int>(std::count_if(data->begin(), data->end(), [avg](auto const i) { return i > avg; })) + 1;
}
...
inline auto activity_counter(pillar_men_sensor const* const s, size_t const len) {
    return std::accumulate(s, s + len, 0, [](auto const tot, auto const& sen) { return tot + sen.activity; });
}
